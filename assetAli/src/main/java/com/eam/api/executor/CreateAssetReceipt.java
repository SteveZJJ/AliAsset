...
public class CreateAssetReceipt implements Callable {
	
	private Receipt_CreateHeader header;
	HttpServletResponse response;
	
	public CreateAssetReceipt(Receipt_CreateHeader header,HttpServletResponse response){
		this.header = header;
	...
	@Override
	public Object call(){
		AliResultProcedure res1 = GetService.getInstance().getMybatisReceiptService().createAssetReceipt(header);
